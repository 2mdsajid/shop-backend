...
import { checkoutTokenRequestSchema, TCheckoutItemInput } from './checkout.schema';
import * as CheckoutServices from './checkout.services'; // Import the product services

const router = express.Router();

// generate checkout token and send to FE 
router.post('/generate-checkout-token', async (request: Request, response: Response) => {
    try {
        ...
        const productsExist = await Promise.all(
            request.body.map(async (item: TCheckoutItemInput) => {
...
router.get('/get-checkout-token-items-details/:token', async (request: Request, response: Response) => {
    try {
