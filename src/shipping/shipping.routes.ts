...
router.post('/create-shipping-address',checkUserExists, async (request: RequestWithUser, response: Response) => {
    try {
  ...
  router.get('/get-shipping-address',checkUserExists, async (request: RequestWithUser, response: Response) => {
    try {
      const userId = request.user?.id;
      if (!userId) {
        return response.status(404).json({ data: null, message: 'User not found' });
