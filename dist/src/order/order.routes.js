...
router.post('/create-order', checkUserExists, async (request, response) => {
    try {
...
router.get('/confirm-order-token-and-get-details/:token', checkUserExists, async (request, response) => {
    try {
...
// get single order
router.get('/get-order-by-id/:orderId', async (request, response) => {
    try {
...
router.post('/update-order-status/:orderId', async (request, response) => {
    try {
        const orderId = request.params.orderId;
        if (!orderId || !isUUID(orderId)) {
            return response.status(401).json({ data: null, messsage: 'Missing Some Parameters!' });
...
router.get('/get-orders-by-user-id/:userId', async (request, response) => {
    try {
